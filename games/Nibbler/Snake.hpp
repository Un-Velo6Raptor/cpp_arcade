...
namespace ar {
	...
	private:
		...
		SnakePart createSnakePart(std::size_t row, std::size_t col,
			bool full
		);
		...
		void updateTail(ar::Tile tail);
		void updateSnake(ar::Tile tile, std::size_t row, std::size_t col);

		bool possibleDestination(ar::Tile tile);
		...
		ar::Direction _direction;
		ar::Direction _prevDirection;
		bool _pause;
		bool _classicMode;
		int _score;
		int _timer;
		...
		std::vector<ar::Snake::SnakePart> _snake;
		std::vector<std::vector<ar::Tile>> _movementMap;
		int _startPause;
		int _tmpTimer;
	};
}
